...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        x = self.place
        k = 0
        while x.is_hive == False and k <= self.max_range:
            if x.bees != [] and self.min_range <= k:
                return choose_bee(x.bees)
            x = x.entrance
            k += 1
        return None
        # END Problem 3 and 4
            ...
            while x:
                if x.ant != None:
                    if x.ant.doubled == False:
                        x.ant.damage += x.ant.damage
                        x.ant.doubled = True
                    if x.ant.is_container and x.ant.ant_contained != None and x.ant.ant_contained.doubled == False:
                        x.ant.ant_contained.damage += x.ant.ant_contained.damage
                        x.ant.ant_contained.doubled = True
                x = x.exit
        # END Problem EC
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.forward_direction == True:
            destination = self.place.exit
        elif self.forward_direction == False and self.place.is_hive == False :
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        if target:
            target.apply_status(Bee.slow, target.action, 3)
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if target and target.scared_before == False:
            target.apply_status(Bee.scare, target.action, 2)
...
class LaserAnt(ThrowerAnt):
        ...
        while x.is_hive == False:
            ...
            if x.ant != None:
                ...
                if x.ant.is_container and x.ant.ant_contained != None:
                    dict[x.ant.ant_contained] = counter
            x = x.entrance
            counter += 1
        return dict
        # END Problem Optional 3
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 3
        return max(0,(2 - self.insects_shot/16)-distance*0.25)
        # END Problem Optional 3
